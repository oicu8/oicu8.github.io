...
export default class Edit extends Component<Props, State> {
  ...
  constructor(props: Props) {
    super(props);
    this.state = {
      code: window.localStorage["markdown-cache"] || `lifechain...`,
      previewIsOn: false,
      topics: [],
      hiddenTopics: {},
      mentions: [],
  ...
  componentDidMount() {
    ...
    utility.checkUserRegistration(this.props.lifechain);
    this.checkTopicsAndMentions();
    this.analyzeReplies();
    document.body.style.overflow = "hidden";
    this.setState({
      postToLifeChainTopic: this.props.lifechain.settings.postToLifeChainTopic
    });
  }
  ...
  async analyzeReplies() {
    ...
    while (transactionInfo) {
      ...
      const name = (await this.props.lifechain.getUserInfoFromAddress(address))
        .name;
      if (!exists[address]) {
        exists[address] = true;
      ...
      if (transactionInfo.decodedInputData.name === "post") {
        ...
        const parentTransactionHash =
          transactionInfo.decodedInputData.params["parentTransactionHash"]
            .value;
        transactionInfo = await this.props.lifechain.getTransactionInfo({
  ...
  private postFeed = async () => {
    ...
    if (this.state.postToLifeChainTopic) {
      topics.push("lifechain");
    }
    ...
    try {
      ...
      this.setState({ generatingIPFSHash: true }, async () => {
        const ipfsHash = (await this.props.lifechain.ipfsAdd(content)).hash;
        ...
        try {
          if (this.props.parentFeedInfo) {
            ...
            // post
            await lifechain.postFeed(digest, hashFunction, size, tags);
          }
                ...
                <div className="topics-and-mentions card">
                  ...
                  <div className="config-list">
                    ...
                    <div
                      className={
                        "config" +
                        (this.state.postToLifeChainTopic ? "" : " hidden")
                      }
                      onClick={() =>
                        this.setState(
                          {
                            postToLifeChainTopic: !this.state.postToLifeChainTopic
                          },
                          () => {
                            this.props.lifechain.settings.postToLifeChainTopic = this.state.postToLifeChainTopic;
                            this.props.lifechain.setSettings(
                              this.props.lifechain.settings
                            );
                          }
                        )
                      }
