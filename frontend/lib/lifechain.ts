...
export class LifeChain {
  ...
  public async upvote(
    ...
    //       https://github.com/oicu8/lifechain/issues/4
    let tags = [];
  ...
  * authorAddress ${authorAddress}`);
  ...
  /**
   * Sync one block, store all lifechain related transaction information into database.
  ...
  public async getTransactionInfo(
    {
      ...
      transactionHash = "",
      maxCreation = 0,
      // TODO: there might be multiple lifechain transaction in one block,
  ...
  public async setUserMetadata(userInfo: UserInfo) {
    ...
    // delete userInfoCopy["username"]; // no need to delete this, because user might want @LifeChain instead of @lifechain.
    if (
  ...
  public initializeDefaultSettings(): Settings {
    ...
    /*
    if (this.userInfo.username !== "lifechain") {
