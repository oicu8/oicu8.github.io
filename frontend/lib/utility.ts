...
import { LifeChain } from "./lifechain";
...
export async function checkUserRegistration(lifechain: LifeChain) {
  const username = await lifechain.getUsernameFromAddress(lifechain.accountAddress);
  if (!username || !username.length) {
    hashHistory.replace(`/${lifechain.networkId}/signup`);
  }
}

export function checkNetworkId(lifechain: LifeChain, networkId: number) {
  console.log(lifechain.networkId, networkId);
  console.log(
    lifechain.getNetworkName(lifechain.networkId),
    lifechain.getNetworkName(networkId)
  );
  if (lifechain.networkId !== networkId) {
    new window["Noty"]({
      type: "success",
      text: i18n.t("notification/wrong-networkId", {
        given: lifechain.getNetworkName(lifechain.networkId),
        required: lifechain.getNetworkName(networkId)
      }),
      timeout: 10000
    }).show();
    return false;
