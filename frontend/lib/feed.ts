...
import { UserInfo, LifeChain } from "./lifechain";
...
export async function getTopicsAndMentionsFromHTML(
  ...
  for (let i = 0; i < tagElems.length; i++) {
    ...
    if (tagElem.classList.contains("tag-mention")) {
      ...
      const userInfo = await lifechain.getUserInfoFromUsername(mention);
      mentions.push({
...
export async function generateSummaryFromHTML(
  ...
  async function renderMedias(div) {
    const medias = div.getElementsByClassName("lifechain-media");
    let video = "";
...
export async function generateFeedInfoFromTransactionInfo(
  ...
  if (feedType === "post") {
    const o = await lifechain.retrieveMessage(
      transactionInfo.decodedInputData.params
    );
    message = o.message;
    ipfsHash = o.ipfsHash;

    summary = await generateSummaryFromHTML(renderMarkdown(message), lifechain);

    userInfo = await lifechain.getUserInfoFromAddress(transactionInfo.from);
  } else if (feedType === "upvote") {
    const repostUserAddress = transactionInfo.from;
    // Get parent transactionInfo
    transactionInfo = await lifechain.getTransactionInfo({
      transactionHash:
        transactionInfo.decodedInputData.params["parentTransactionHash"].value
    });

    // who reposts the feed
    repostUserInfo = await lifechain.getUserInfoFromAddress(repostUserAddress);
  ...
  const stateInfo = await lifechain.getFeedStateInfo(transactionInfo.hash);
